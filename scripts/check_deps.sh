...
# markitdown — auto-install if missing
PIP_CMD=""
...
if python3 -c "import markitdown" 2>/dev/null || python -c "import markitdown" 2>/dev/null; then
  ...
  echo -e "$WARN markitdown missing — installing..."
  ${PIP_CMD:-pip3} install "markitdown[pptx]" Pillow defusedxml -q 2>/dev/null
  if python3 -c "import markitdown" 2>/dev/null; then
    echo -e "$PASS markitdown (installed)"
  else
    echo -e "$FAIL markitdown — auto-install failed. Run: ${PIP_CMD:-pip3} install \"markitdown[pptx]\" Pillow defusedxml"
    blockers=$((blockers + 1))
  fi
fi

# poppler (pdftoppm) — auto-install if missing
if command -v pdftoppm >/dev/null 2>&1; then
  echo -e "$PASS poppler (pdftoppm)"
else
  echo -e "$WARN poppler missing — installing..."
  brew install poppler -q 2>/dev/null
  if command -v pdftoppm >/dev/null 2>&1; then
    echo -e "$PASS poppler (installed)"
  else
    echo -e "$FAIL poppler — auto-install failed. Run: brew install poppler"
    blockers=$((blockers + 1))
  fi
fi

# LibreOffice — prompt if missing (large install, can't auto-install silently)
if command -v soffice >/dev/null 2>&1; then
  echo -e "$PASS LibreOffice"
else
  echo -e "$FAIL LibreOffice — not found. Run: brew install --cask libreoffice"
  blockers=$((blockers + 1))
fi
...
if [ $blockers -gt 0 ]; then
  ...
  echo -e "Result: Ready. $warnings tool(s) were auto-installed (see above)."
  exit 0
else
